...
addition_value = parameters.gap_value
...
buff_size_LE_2_value = 0.3 + my_utils.calculate_real_size(map_scale, contours_size_3)
...
arcpy.AddField_management (basic_line_02, 'id_basic_line', "SHORT")
u_cursor = arcpy.da.UpdateCursor(basic_line_02, ['OBJECTID','id_basic_line'])
for row in u_cursor:
    id = row[0]
    row[1]= id
...
basic_line = arcpy.Dissolve_management ('singlepart_lyr', 'basic_line_3', 'id_basic_line')
# ZMENY SEM
...
buffers = arcpy.Buffer_analysis('model_bl_lyr', buffers_CL, gap, 'LEFT', 'FLAT', 'ALL', '', 'PLANAR')
...
for id in basic_lines_list: #_select:
    ...
    gap_value = parameters.gap_value
    gap = '{} Meters'.format(gap_value)
    cheek_value = 0.2 + parameters.gap_value
    cheek = '{} Meters'.format(cheek_value)
    count_cl = int(maximal_width_of_wall / gap_value)
    ...
    try:
        for i in range(1, (count_cl + 1)):
            ...
            if i == count_cl:
                ...
                arcpy.CalculateField_management(one_buff, 'id_tc', id)
